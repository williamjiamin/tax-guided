...
import type { TaxShort } from "@/lib/shorts/shorts-data";
...
export function ShortsPageClient({ shorts, filters }: ShortsPageClientProps) {
  const [activeFilter, setActiveFilter] = useState("all");

  const filtered =
    activeFilter === "all"
      ? shorts
      : shorts.filter((s) => s.filters.includes(activeFilter));
        ...
        <section className="bg-gradient-to-b from-navy-dark to-navy pt-20 pb-14 px-6">
          <div className="max-w-3xl mx-auto text-center">
            ...
            <h1 className="text-[2.5rem] md:text-[3.25rem] font-extrabold text-white mb-5 leading-[1.1] tracking-tight">
              Tax Concepts in{" "}
              <span className="text-gold">Under 60 Seconds</span>
            </h1>
            <p className="text-lg text-gray-300 mb-2 max-w-xl mx-auto leading-relaxed">
              Each short breaks down one tax topic — the hook, the explanation,
              and the action step — so you can learn fast and move on.
            </p>
          </div>
        </section>
            ...
            {filtered.map((short) => (
              ...
              >
                ...
                <div className="flex items-center gap-2 mb-3">
                  <span className="inline-block bg-navy text-gold text-[11px] font-bold px-2.5 py-1 rounded-full">
                    {short.duration}
                  </span>
                  <span className="inline-block bg-navy/5 text-navy text-xs font-semibold px-2.5 py-1 rounded-full group-hover:bg-gold/10 group-hover:text-gold-dark transition-colors">
                    {short.formatLabel}
                  </span>
                </div>
          ...
          {filtered.length === 0 && (
            <div className="max-w-5xl mx-auto text-center py-16">
              <p className="text-gray-400 text-lg">
                No shorts match this filter yet.
              </p>
            </div>
          )}
        </section>
